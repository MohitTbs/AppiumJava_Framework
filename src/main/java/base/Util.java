...
public class Util {
	...
	public static void scrollNclick(By listItems, String attrName, String text) throws InterruptedException {
		...
		while (!isEndOfPage(prevPageSource)) {
			...
			for (WebElement el : AppDriver.getCurrentDriver().findElements(listItems)) {
				...
				if (el.getDomAttribute(attrName).equalsIgnoreCase(text)) {
					el.click();
	...
	public static void scrollNclickWithRatio(By byEl, double ScrollRatio) {
		String prevPageSource = "";
		boolean flag = false;
		...
		while (!isEndOfPage(prevPageSource)) {
			...
			try {
				...
				scroll(ScrollDirection.DOWN, ScrollRatio);
			}
		}
...
//    }
	...
	public static void clickOnBackButton() {
		...
		if (AppDriver.getCurrentDriver() instanceof AndroidDriver) {
			((AndroidDriver) AppDriver.getCurrentDriver()).pressKey(new KeyEvent().withKey(AndroidKey.BACK));
		} else if (AppDriver.getCurrentDriver() instanceof IOSDriver) {
